...
class EntriesViewController: UIViewController, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    // Sectioned data (Breakfast -> Lunch -> Dinner -> Snacks)
    private var sectionedData: [(meal: MealType, items: [FoodLogData])] = []

    private func rebuildSections() {
        let orderedMeals: [MealType] = [.breakfast, .lunch, .dinner, .snacks]

        sectionedData = orderedMeals.map { meal in
            let items = data
                .filter { $0.meal == meal }
                // keep stable ordering inside a meal section (change if you prefer time, etc.)
                .sorted { $0.name.localizedCaseInsensitiveCompare($1.name) == .orderedAscending }
            return (meal: meal, items: items)
        }
        // Hide empty sections. Remove this filter if you want empty headers too.
        .filter { !$0.items.isEmpty }

        emptyLabel.isHidden = !data.isEmpty
    }

    private func item(at indexPath: IndexPath) -> FoodLogData {
    ...
    func refresh() {
        CloudManager.shared.getAll(for: HomeViewController.selectedDate) { logs in
            self.data = logs
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        // empty state handled by rebuildSections()
        return sectionedData[section].items.count
    }

    func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        // Your enum raw values are already "Breakfast", "Lunch", etc.
        return sectionedData[section].meal.rawValue
    }
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if let resultCell = cell as? EntryCell {
            resultCell.refresh(with: self.item(at: indexPath))
        }
    }
...
extension EntriesViewController {
    ...
    class EntryCell: UITableViewCell {
        ...
        override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
            ...
            myFoodIcon.image = UIImage(systemName: "bookmark.fill")?
                .withConfiguration(UIImage.SymbolConfiguration(font: .systemFont(ofSize: 17, weight: .semibold)))
