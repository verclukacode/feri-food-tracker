...
final class HomeViewController: UIViewController, ScanEANViewControllerDelegate {

    static var selectedDate: Date = .now
    static var activeObject: HomeViewController?
    ...
    override func viewDidLoad() {
        ...
        HomeViewController.activeObject = self
        ...
        datePicker.addAction(UIAction(handler: { [weak self] _ in
            ...
            HomeViewController.selectedDate = self.datePicker.date
            HomeViewController.activeObject?.refresh()
        }), for: .valueChanged)
        datePicker.date = HomeViewController.selectedDate
        ...
        addButton.menu = UIMenu(children: [
            UIAction(title: "Browse food", image: UIImage(systemName: "magnifyingglass"), handler: { [weak self] _ in
                guard let self else { return }
                let viewController = LogFoodBrowseViewController(selectedDate: HomeViewController.selectedDate) {
                    self.refresh()
                }
    ...
    override func viewDidLayoutSubviews() {
        ...
        backView.frame = CGRect(
            x: 0,
            y: viewEntriesButton.frame.minY - 30,
            width: view.frame.width,
            height: (view.frame.height - viewEntriesButton.frame.minY - 30 + view.safeAreaInsets.bottom + 100)
        )
        
        caloriesLabel.frame = CGRect(
            x: (view.frame.width - 120) / 2,
            y: (backView.frame.minY) / 2,
            width: 120,
            height: 120
        )
...
extension HomeViewController {
    ...
    func refresh() {
        CloudManager.shared.getAll(for: HomeViewController.selectedDate) { [weak self] foodLogs in
            guard let self else { return }
            ...
            DispatchQueue.main.async {
                ...
                if foodLogs.isEmpty {
                    self.viewEntriesButton.setTitle("No entries yet", for: .normal)
                } else if foodLogs.count == 1 {
                    self.viewEntriesButton.setTitle("1 Entrie", for: .normal)
                } else {
                    self.viewEntriesButton.setTitle("\(foodLogs.count) Entries", for: .normal)
                }
            }
        }
    }
